...
def clip_board(img: bytes) -> None:
    """
    Take image from arguments (bytes returned from function screenshot) and place it to clipboard using xclip
    """
    clip = subprocess.run(["xclip", "-selection", "clipboard", "-t", "image/png"], input=img)
